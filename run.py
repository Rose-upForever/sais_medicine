...
class RNADataset(Dataset):
    def __init__(self, npy_dir):
        ...
        # 获取目录下所有.npy文件名（不含扩展名作为pdb_id）
        self.name_list = [f.split('.')[0] for f in os.listdir(npy_dir) if f.endswith('.npy')]
    ...
    def __getitem__(self, idx):
        ...
        # 假设.npy文件中原子顺序固定为: P, O5', C5', C4', C3', O3'
        feature = {
            "name": pdb_id,
            "seq": "A" * coords.shape[0],  # 用占位符'N'表示未知序列
            "coords": {
                "P": coords[:, 0, :],
                "O5'": coords[:, 1, :],
                "C5'": coords[:, 2, :],
...
class ModelConfig:
    ...
    k_neighbors: int = 50  # 明确指定为 int 类型
    dropout: float = 0.1
    ...
    num_decoder_layers: int = 4  # 修正为整数，去掉多余的小数点
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader):
            ...
            for length,name in zip(lengths,names):
                ...
                char_seq = ''.join([alphabet[num] for num in sample_list])
                print(char_seq)
                result["pdb_id"].append(name)
                result["seq"].append(char_seq)
                start_idx = end_idx
